<template name="graceSwiper">
	<view>
		...
		<swiper class="grace-swiper" autoplay="true" :indicator-dots="indicatorDots" indicator-color="rgba(255, 255, 255, 1)" indicator-active-color="#00B26A" :interval="interval">
			<swiper-item class="swiper-item" v-for="(item, index) in items" :key="index">
				<navigator class="" :url='item.path' :open-type="item.openType">
					<image class="swiper-img" :src='item.imgUrl' @load='imgLoad' :id="swiperId + '-item-1'" mode='aspectFill'></image>
...
<style>
.grace-swiper{width:100%; height:750upx; position:relative;}
.grace-swiper .swiper-item{width:100%; height: 100%; font-size:0; line-height:0;}
.grace-swiper .swiper-item .swiper-img{width:750upx;height:750upx;}
