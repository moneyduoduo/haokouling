		...
		<view v-if="show">
			...
			<view class="yj-content yj-mainF">
				...
				<view class="yj-bg-fff yj-pad-lr yj-pad-b-sm">
					<view class="yj-display-flex yj-middle">
						<view class="yj-goods-title yj-pad-t-min yj-flex-1" @longpress="copyinfo(goodsInfo.itemTitle)">
							<image class="goodsTitleIcon" src="/static/image/index/pddIcon.png"></image>
							{{goodsInfo.itemTitle}}
						</view>
					...
					<view class="yj-display-flex yj-between yj-mar-t-min">
						<view class="yj-color-main">
							券后 &yen; <text class="yj-goods-pirce yj-text-super">{{itemEndPrice}}</text>
							<text class="yj-del yj-color-gray-light yj-text-sm yj-mar-l-sm">&yen;{{goodsInfo.itemGroupPrice}}</text>
						</view>
						<view class="yj-zhuan-2" v-if="!isReview">赚&yen;{{goodsInfo.zhuan}}</view>
					</view>

					<view class="detail_bigquan yj-mar-t-sm" @click="goUrl()">
						<view class="money">
				...
				<view class="grace-product-info">
					<image v-for="(item,index) in pic" :key="index" :src="item.url" mode="widthFix" alt="00"></image>
				</view>
			</view>
			...
			<view class="yj-footer yj-pad-tb-min yj-bg-fff">
				...
				<view class="yj-flex-1 yj-display-flex yj-text-center yj-radius-sm yj-mar-r yj-text-bold" style="overflow: hidden;">
					<view class="yj-flex-1 yj-bg-yellow-dark yj-display-flex yj-middle"  @click="showCanvas()">
						<view class="yj-text-biglotter yj-flex-1">
							&yen;{{goodsInfo.zhuan}}
							<view class="yj-text-sm">分享赚</view>
						</view>
					</view>
					<view class="yj-flex-1 yj-bg-main yj-display-flex yj-middle" @click="goUrl()">
						<view class="yj-text-biglotter yj-flex-1">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.goodsShareFn({itemId:this.itemId}).then(res=>{
				this.inviterCode = res.data.mobileShortUrl
			})
		},
		onshow(){
		...
		methods: {
			async favourite(itemId){
				...
				if(islogin){
					...
					let data = await this.$http.post('/api/goodsFavourite/doCollect',{itemId:this.itemId,platformType:3}) 
					this.collect = data.data.isFavorite
					console.log(data)
				}else{
					this.$msg.confirm({ title:'提示',content: '请先登录'},()=>{
						uni.navigateTo({
							url: `/pages/pass/login?back=pages/pdd/detail/index?itemId=${itemId}`
						});
					})
				}
			},
			...
			goodsShareFn(obj){
				return new Promise((resolve,reject)=>{
					this.$http.post('/api/pddGoods/getPromotionUrl',obj)
					.then(res =>{
						resolve(res)
					}).catch(err=>{
						console.log(err)
			...
			async goodsBaseInfo(itemId){
				// 淘宝比例 *  券后价 * 接口比例 / 100
				let data = await this.$http.post('/api/pddGoods/getGoodsDetail',{itemId:itemId})
				let zhuan = ((parseFloat(data.data.itemGroupPrice)-data.data.couponMoney)*data.data.pddRates/1000*this.bili).toFixed(2)
				...
				this.shopType = '/static/image/index/pddIcon.png'
				this.itemTitle = data.data.itemTitle
				this.itemSale = data.data.itemSale
				this.couponMoney = data.data.couponMoney
				this.itemEndPrice = (data.data.itemGroupPrice - data.data.couponMoney).toFixed(2)
				this.itemPrice = data.data.itemGroupPrice
			...
			shareWeixinOne(){ //分享给微信朋友
				...
				let inviterUrl = this.inviterCode
				uni.share({
					provider: "weixin",
			...
			goodsImgInfo: function(id) {
				...
				// #ifdef H5
				uni.request({
					url: '/pdd/goods.html?goods_id=' + id,
					dataType: "text",
					success: (rsp) => {
						if(rsp.data.includes('detailGallery')){
							console.log(rsp.data)
							let data = rsp.data.match(/("detailGallery\":=?)(\S*)(?=,"videoGallery")/);
				...
				// #ifdef APP-PLUS
				uni.request({
					url: 'http://mobile.yangkeduo.com/goods.html?goods_id=' + id,
					dataType: "text",
					success: (rsp) => {
						if(rsp.data.includes('detailGallery')){
							let data = rsp.data.match(/("detailGallery\":=?)(\S*)(?=,"videoGallery")/);
			...
			//分享赚||领取优惠券
			goUrl:function(){ // 0 领优惠券 1 跳转商品详情  2 分享
				this.goodsShareFn({itemId:this.itemId}).then(res=>{
					uni.navigateTo({
						url: '/pages/common/webView?title=拼多多&url='+ encodeURIComponent(res.data.mobileShortUrl)
					})
				})
...
<style lang="less">
	...
	.share_box .mark{background-color:rgba(0,0,0,0.5); position: fixed; top:0;left:0; width: 100%; height:100%; z-index: 10000;}
	.share_box .sharePostion{position: fixed; top:0;left:0; width: 100%; height:100%; z-index: 10001; display: flex;justify-content: center;align-items: center;}
	.share_box .sharePostion .shareImg{width: 80%; height: 90%; position: relative;}
	.share_box .sharePostion .shareImg .cancel{width: 15px; height:15px;position: absolute;top:5px;right:10px;z-index: 10;}
	.share_box .sharePostion .shareImg .cancel image{width: 100%;height:100%;}
	.share_box .sharePostion .shareImg .img_box{background-color: #ffffff;border-radius: 10px;height:80%;padding:20px 10px;}
	.share_box .sharePostion .shareImg .img_box image{width: 100%;height:100%;}
	.share_box .sharePostion .shareImg .btn_box{display: flex;justify-content: center;align-items: center; margin-top: 10px;}
	.share_box .sharePostion .shareImg .btn_box .btn{width: 120px; height:35px;text-align: center;background-color: #FD4C66;color: #fff;line-height: 35px; margin-left: 10px; border-radius: 10px;}
